...
int main (int argc, char **argv)
{
    ...
    snprintf(results, MAX_PATH, "ResultadosNSGA2/%s/%f.out", instance, seed);
    snprintf(times, MAX_PATH, "TiemposNSGA2/%s.out", instance);
    snprintf(path, MAX_PATH, "Instancias/%s.dat", instance);

    fpt3 = fopen(results,"w");
    ...
    */
    ...
    /*
    printInd(parent_pop[0].ind);
    repair_ind(parent_pop[0].ind,pi);
    printInd(parent_pop[0].ind);
    */

    /*decode_pop(parent_pop); Creo que no es necesario en mi caso*/
    evaluate_pop (parent_pop,pi);
    assign_rank_and_crowding_distance (parent_pop); /*No se si esto esta bien*/
    ...
    printf("\n Time taken for execution = %f seconds", time_spent);
    fprintf(fpt6, "%f\n", time_spent);
